            ...
            {cartItems.length <= 0 ? (
                <div className='emptyCartDiv'>
                    <div className="emptyCart">
                        <p className='emptyCartContentText'>Your AgriX Cart is empty.</p>
                ...
                <div className="cartContainer">
                    <div className="cartBoxContainer">
                        ...
                        {cartItems.map(product => (
                            <div key={product.product} className="cartBox">
                                ...
                                <div className="cartProductPriceDiv">
                                    <p className="cartProductPrice">Rs.{toPrice(product.price)}</p>
                                </div>
                        ...
                        <div className="subTotalCartDiv">
                            <p className="subTotalCartText"><span className='cartSubTotalText'>Subtotal ({cartItems.reduce((a, c) => a + c.qty, 0)} {cartItems.length <= 1 ? "item" : "items"}) : </span><span className='cartSubTotalPrice'> Rs.{toPrice(cartItems.reduce((a, c) => a + c.price * c.qty, 0))}</span></p>
                        </div>
                    </div>
                    <div className="checkoutBox">
                        <p className="subTotalText"><span className='checkOutSubTotalText'>Subtotal ({cartItems.reduce((a, c) => (a + c.qty), 0)} {cartItems.length <= 1 ? "item" : "items"}) : </span><span className='checkOutSubTotalPrice'> Rs.{toPrice(cartItems.reduce((a, c) => a + c.price * c.qty, 0))}</span></p>
